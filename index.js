...
function PascalCoind(_opts) {
    ...
    this.on("started", () => {
        ...
        if(opts.minerMonitor) {
            setTimeout(() => {
                // quick timeout before connecting to miner port
    ...
    this.doHeartbeat = function() {
        _this.sendRpc("nodestatus", {}, (data) => {
            if(data.hasOwnProperty("error")) {
                ...
                _this.failCount = 0;
                _this.emit("info", data.result.ready_s);
                _this.emit("info", "blockcount: %s, connected to %s peers", [data.result.blocks, data.result.netstats.active]);
